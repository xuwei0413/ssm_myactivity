...
public class UserServiceImpl implements IUserService {
    ...
    public void updateUser(UserVo userVo) {
        if(StringUtil.isNotEmpty(userVo.getPwd())){
            userVo.setPwd(DigestUtils.md5DigestAsHex(userVo.getPwd().getBytes()));
        }
    ...
    public Integer checkUser(UserVo userVo) {
        Integer num = userMapper.selectByName(userVo);
        if(num>0){
            return  -1;
        }else{
            return 0;
    ...
    public Integer addUser2(UserVo userVo) {
        ...
        //设置用户类型 都是普通用户 type=2
        userVo.setType(SysConstast.USER_TYPE_NORMAL);
        if(this.userMapper.insertSelective(userVo)>0){
            return 0;
        }else{
            return 1;
        }
    ...
    @Override
    public DataGridView getUserInfo(User user) {
        List<User> data = this.userMapper.queryAllUser(user);
        return new DataGridView(null,data.get(0));
    }
